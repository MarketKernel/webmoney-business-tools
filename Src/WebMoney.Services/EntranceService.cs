    ...
    public sealed class EntranceService : IEntranceService
    {
        ...
        private static void UpdateDatabase(IConnectionSettings connectionSettings)
        {
            ...
            switch (connectionSettings.ProviderInvariantName)
            {
                case DataConfiguration.SqlServerProviderInvariantName:
                    migrationsDirectory =
                        Path.Combine(DbMigrationsConfiguration.DefaultMigrationsDirectory,
                            nameof(Migrations.SqlServer));
                    migrationsNamespace = typeof(Migrations.SqlServer.V1).Namespace;
                    break;
                case DataConfiguration.SqlServerCompactProviderInvariantName:
                    migrationsDirectory =
                        Path.Combine(DbMigrationsConfiguration.DefaultMigrationsDirectory, nameof(Migrations.SqlCE));
                    migrationsNamespace = typeof(Migrations.SqlCE.V1).Namespace;
                    break;
                default:
            ...
            configuration.MigrationsDirectory = migrationsDirectory;
            configuration.MigrationsNamespace = migrationsNamespace;
